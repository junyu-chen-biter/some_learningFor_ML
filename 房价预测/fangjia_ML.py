...
#这里是去设置了这个字典，键值对没问题的，值是一个元组，里面是下载的位置
...
all_features=pd.get_dummies(all_features,dummy_na=True) #转化为热独编码


#转化为pytorch张量
all_features = all_features.astype(np.float32)
n_train=train_data.shape[0]#确实是包含了目标变量的
...
#k是总共由几折，i是当前是第几折，X是train的tensor数据，Y是train的目标值（房价）
def get_k_fold_data(k,i,X,y):
    assert k>1
    fold_size=X.shape[0]//k
    X_train,y_train=None,None
    for j in range(k):
        idx=slice(j* fold_size,(j+1)*fold_size)#这个索引会每次向后移动一定的位置
...
#X_train是train的tensor数据，Y_train是train的目标值（房价），其他都是参数
def k_fold(k,X_train,y_train,num_epochs,learning_rate,weight_decay,batch_size):
    train_l_sum,valid_l_sum=0,0 #两个初始化的损失
    for i in range(k):
...
def train_and_pred(train_features, test_features, train_labels, test_data,
    ...
    d2l.plot(np.arange(1, num_epochs + 1), [train_ls], xlabel='epoch',
             ylabel='log rmse', xlim=[1, num_epochs], yscale='log') #画图的
...
k,num_epochs,lr,weight_decay,batch_size=7,100,5,0,64
train_l,valid_l=k_fold(k,train_features,train_labels,num_epochs,lr,weight_decay, batch_size)
print(f'{k}-折预测：平均训练log rmse：{float(train_l):f},'f'平均验证log rmse：{float(valid_l):f}')
plt.show()
